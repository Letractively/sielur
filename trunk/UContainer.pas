...
type
  ...
  public
    ...
    procedure MyNavigate(const URL: WideString);
    procedure MyFormSubmit(const MyForm: IHTMLFormElement);
    procedure MyElementClick(const MyElement: IHTMLElement);
...
function TWBContainer.GetOptionKeyPath(var pchKey: POLESTR;
  const dw: DWORD): HResult;
var
  ResultLen: Integer;
begin
  Result := S_FALSE;
  // В поле FOptionKeyPath: String
  ...
  begin
    ...
    begin
      MultiByteToWideChar(CP_ACP, 0, PAnsiChar(FOptionKeyPath), -1,
        pchKey, ResultLen);
      Result := S_OK;
    end;
  end else begin
