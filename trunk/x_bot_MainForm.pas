  ...
  begin
    ...
    if Accounts_TreeView.Items.Count <= 0 then ACF.Account_data:=nil
      ...
      if  (PNodeData(Node.Data)^.NodeType = -3) or ((PNodeData(Node.Data)^.NodeType = -2) and  PNodeData(Node.Data)^.Status) then
        ACF.Account_data:=PNodeData(Node.Data)^.Account_Data  // Это Account и он залогинен
      else
...
begin
  ...
  try
    ...
    try
      ini.ReadSections(LS);      // читаем все секции в список (на самом деле это наши сервера)
      ...
      try
        ...
        begin
          ...
          ini.ReadSection(Server_Name, LV);  // читаем список ключей (имен) текущей секции
