...
Type
  TPanelControl = class
    ...
    FPanel: TPanel;         //главна€ панель
    FImage: TImage;         //картинка постройки
    FImageList: TImageList; //контейнер дл€ плюса и минуса на кнопках BtAdd и BtDel
...
Type
  ...
  public
    ...
    //AcolGID - количество создаваемых панелек по √»ƒ постройкам ...
...
begin
  ...
  //заранем знаем скок их там (помен€тьс€ может если помен€ют версию клиента)
  //дл€ большего удобства последовательность панелек будет фиксированой
...
begin
  ...
  try
    ...
    //передаем его в форму где ему присво€т актуальные параметры
    ...
    FarmItem.Id := NextId;
    //добовл€ем в список фарма фарм елемент с коректным айдишником и данными
    Account_data.MyAccount.Derevni.VillById(Account_data.MyAccount.IdCurrentVill).FarmLists.AddFarmItem(FarmItem);
    //отображаем на форме  токо часть дл€ теста
...
// ¬озвращает »ндекс строени€ (дл€ полей) над которым стоит курсор мышки
var
  IdCurrentVill: Integer;
  CurrentVill: TVill;
  CIndex: Integer;
