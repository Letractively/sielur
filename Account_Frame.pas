...
uses
  ...
  Dialogs, StdCtrls, OleCtrls, SHDocVw, ExtCtrls, RzPanel, RzTabs,
  UContainer,Account_data,
  Trava_Class, RzSplit, Grids, RzGrids, ImgList, pngimage;
...
begin
  ...
  if img_index > 0 then
  begin
    if (CIndex = 40) and  (CurrentVill.Item_Building[40].lvl = 0) then img_index:=0;
...
begin
  ...
  if VillForDraw.Item_Building[40].lvl = 0 then
    ...
    img_fn:='image/center/bg'+IntToStr(Account_data.MyAccount.Race)+'15.png';
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Building_ImageList.Draw(VillCenterImage.Canvas,Field_coord[1,CIndex]+25-iw, Field_coord[2,CIndex]+25-ih,Build_index,true);
      end;
    end;

    Level_ImageList.Draw(VillCenterImage.Canvas,Field_coord[1,CIndex]-10, Field_coord[2,CIndex]-15,Lvl_index,true);
...
begin
  ...
  begin
    Lvl_index:=VillForDraw.Item_Building[CIndex].lvl;
    Level_ImageList.Draw(VillFieldImage.Canvas,Field_coord[1,CIndex]-4, Field_coord[2,CIndex]-4,Lvl_index,true);
...
procedure TAccount_Form.VillFieldImageMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var
  r : integer;
  c : integer;
...
begin
  ...
  for CIndex := 1 to 18 do
  begin
    if ((X-Field_coord[1,CIndex])*(X-Field_coord[1,CIndex])+(Y-Field_coord[2,CIndex])*(Y-Field_coord[2,CIndex]))<400 then
    begin
